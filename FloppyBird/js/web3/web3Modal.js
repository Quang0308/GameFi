...
const FLOPPY_ADDRESS = '0x3cA0af153988bAB6F79b75d6917049F9AAbeD8Fe';
const FLOPPY_ABI = [
	{
		"inputs": [],
...
const VAL_ADDRESS = '0xd07d5D6584C99000BC2101bB2C37bE387733C0f0';
const VAL_ABI = [
	{
		"inputs": [],
		"stateMutability": "nonpayable",
...
const TESTNET_POLYGONSCAN = "https://www.oklink.com/vi/amoy/address/"
...
function init() {
  const providerOptions = {
    walletconnect: {
      ...
      options: {
        rpc: {
          80002: 'https://polygon-amoy.g.alchemy.com/v2/MUpeEFamQR5lA_XrfMSK3uaGp0J-3k83',
...
async function fetchAccountData() {
	...
	const balance = await window.ethereum.request({ method: 'eth_getBalance', params: [selectedAccount] });
	...
	// const rs = await startMatch(accInfo.walletAddress);
	// matchId = rs.Id;
...
async function onConvertTicket() {
  ...
  let url = `${TESTNET_POLYGONSCAN}${tranHash}`
...
window.addEventListener('load', async () => {
  init();
  document.querySelector("#connect-button").addEventListener("click", onConnect);
